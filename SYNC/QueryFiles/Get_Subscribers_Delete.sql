...
update [ks_EtlChangeLog]
set is_Processed = 1
,Processed_Time = getdate()
where Id in (
	select Id from @TempLog)
...
left join [dbo].[KS_SubscriberLanguage] sl with(nolock) on s.Subscriber_ID = sl.Subscriber_ID 
where s.UNIT_ID <> 'CANC'